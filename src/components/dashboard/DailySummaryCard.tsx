...
export const DailySummaryCard: React.FC<Props> = ({ totalKg }): React.ReactElement => {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
      <div>
        <h2 className="text-gray-500 text-sm font-medium uppercase tracking-wider mb-1">
          Today&apos;s Footprint
        </h2>
      ...
      <div
        className={`px-4 py-2 rounded-full border text-sm font-bold uppercase tracking-wide ${badgeColors[level]}`}
      >
        {level}
      </div>
    </div>
