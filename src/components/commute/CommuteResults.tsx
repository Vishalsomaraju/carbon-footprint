  ...
  return (
    <div className="lg:col-span-2 space-y-6">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
          <p className="text-gray-500 text-sm uppercase tracking-wider font-bold mb-1">
            Annual Co2
          </p>
          <p className="text-3xl font-bold text-gray-900">
      ...
      {bestAlternative && savings > 10 && (
        <div className="bg-green-50 p-6 rounded-xl border border-green-200">
          <p className="text-green-800">
            <span className="font-bold">💡 Tip:</span> If you switched to{' '}
            ...
            >
              ...
              <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={20}>
                {chartData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.rawKey === mode ? '#10b981' : '#e5e7eb'}
                  />
                ))}
              </Bar>
            </BarChart>
      ...
      <div className="flex justify-end">
        <button
          onClick={onLog}
          disabled={logLoading}
          className="px-6 py-3 bg-carbon-600 text-white rounded-lg font-medium hover:bg-carbon-700 disabled:opacity-50 flex items-center gap-2"
        >
