...
export const activitySchema = z.object({
  ...
  value: z.number({ invalid_type_error: 'Value must be a number' }).positive('Value must be greater than 0.'),
...
export const commuteSchema = z.object({
  ...
  days: z.number({ invalid_type_error: 'Days must be a number' }).min(1, 'Must commute at least 1 day.').max(7, 'Cannot exceed 7 days.'),
});
