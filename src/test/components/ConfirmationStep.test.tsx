...
describe('ConfirmationStep', () => {
  it('renders and handles input', () => {
    ...
    render(<ConfirmationStep data={{ category: 'transport', subCategory: 'car_petrol_per_km', value: 10 }} onSubmit={mockSubmit} onBack={vi.fn()} isLoading={false} />);
  ...
  it('handles errors in factor calculation gracefully', () => {
    render(<ConfirmationStep data={{ category: 'unknown_cat', subCategory: 'unknown_sub', value: 10 }} onSubmit={vi.fn()} onBack={vi.fn()} isLoading={false} />);
    expect(screen.getByText(/Estimated Impact/i)).toBeInTheDocument();
  });
});
